...
type (
	...
	TenantDns struct {
		...
		Namespaces  []string `bson:"namespaces" json:"namespaces" bun:",array"` // In current composite deployment design, Namespaces field of tenants is ignored
...
func (s ServiceRegistration) ToService() *domain.Service {
	annotations := map[string]string{
		"netcracker.cloud/tenant.service.alias.prefix": s.VirtualService,
		"netcracker.cloud/tenant.service.show.name":    strings.Title(strings.ReplaceAll(s.VirtualService, "-", " ")),
	...
	return &domain.Route{
		Metadata: domain.Metadata{
			...
			Annotations: map[string]string{
				...
				"netcracker.cloud/tenant.service.show.name": strings.Title(strings.ReplaceAll(s.VirtualService, "-", " ")),
				"netcracker.cloud/tenant.service.id":        s.VirtualService,
				"netcracker.cloud/tenant.service.type":      "virtual",
			},
		},
...
func (t TenantDns) ToRoute(serviceName string, address Address) *domain.Route {
	...
	return &domain.Route{
		Metadata: domain.Metadata{
			...
			Annotations: map[string]string{"netcracker.cloud/tenant.service.tenant.id": t.TenantId},
		},
		Spec: domain.RouteSpec{
			Host:    address.Host(),
...
func FromRoutes(routes *[]domain.Route) *[]TenantDns {
	...
	for _, route := range *routes {
		tenantId := route.Metadata.Annotations["netcracker.cloud/tenant.service.tenant.id"]
		tr, ok := tmp[tenantId]
		if !ok {
		...
		services[serviceName] = append(addresses,
			ConcatAddress(route.Spec.Host, route.Metadata.Annotations["netcracker.cloud/tenant.service.url.suffix"]),
		)
	}
